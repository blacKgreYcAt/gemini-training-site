...
export default function CoursePage({ params }: { params: Promise<{ week: string }> }) {
  ...
  if (course?.pages) {
    ...
    return (
      <div style={{ background: '#0a0a0a', color: '#fff', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        <div style={{ borderBottom: '2px solid #00aeef', padding: '20px 40px', display: 'flex', justifyContent: 'space-between' }}>
          <button onClick={() => setSelectedId(null)} style={{ background: 'none', border: 'none', color: '#00aeef', cursor: 'pointer', fontSize: '16px' }}>← 返回</button>
        ...
        <section style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '60px 40px' }}>
          <h1 style={{ fontSize: '48px', color: '#00aeef', marginBottom: '40px', textAlign: 'center' }}>{page.title}</h1>
          <div style={{ fontSize: '24px', color: '#ccc', lineHeight: 1.8, whiteSpace: 'pre-wrap', maxWidth: '900px', textAlign: 'left' }}>
            {page.content}
          </div>
        </section>

        <div style={{ borderTop: '2px solid #00aeef', padding: '20px 40px', display: 'flex', justifyContent: 'space-between' }}>
          <button
            onClick={() => setPageIdx(p => Math.max(0, p - 1))}
            disabled={pageIdx === 0}
            style={{ padding: '10px 20px', border: '2px solid #00aeef', background: 'transparent', color: '#00aeef', cursor: pageIdx === 0 ? 'not-allowed' : 'pointer', opacity: pageIdx === 0 ? 0.5 : 1 }}
          >
            ← 上一頁
          </button>
    ...
    <div style={{ padding: '40px', background: '#0a0a0a', color: '#fff', minHeight: '100vh' }}>
      ...
      {courses.map(c => (
        <div
          ...
          style={{ marginBottom: '30px', padding: '25px', border: '2px solid #00aeef', background: '#111', cursor: 'pointer' }}
        >
          <h2 style={{ color: '#00aeef', marginBottom: '10px' }}>{c.title}</h2>
          <p>{c.description}</p>
